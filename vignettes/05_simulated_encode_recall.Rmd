...
The paradigm implements a 2 (task: encode vs. recall) × 11 (video identities)
...
The top $r$ eigenvectors in $W$ represent the dominant **communication modes**—
...
**mppi_reinstatement()** tests the **neural reinstatement hypothesis**—that
...
**mppi_sdppi()** analyzes **state-dependent psychophysiological interactions**—how
...
providing insights into effective connectivity—the direct connections between
...
task-only baseline, call `mppi_evidence()`. Positive ΔAIC/ΔBIC values indicate
